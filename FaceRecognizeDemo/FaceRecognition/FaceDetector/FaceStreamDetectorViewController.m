...
- (void)viewDidDisappear:(BOOL)animated
{
    ...
    //上传照片失败
    if ([self.faceDelegate respondsToSelector:@selector(sendFaceImageError)] && imageView.image==nil) {
...
-(void)makeUI
{
    ...
    //提示框
    imgView = [[UIImageView alloc]initWithFrame:CGRectMake((ScreenWidth-ScreenHeight/6+10)/2, CGRectGetMaxY(self.previewView.frame)+15*kHEIGHT/667.0, ScreenHeight/6-10, ScreenHeight/6-10)];
    [self.view addSubview:imgView];
    
    self.textLabel = [[UILabel alloc]initWithFrame:CGRectMake((ScreenWidth-150)/2, CGRectGetMaxY(imgView.frame)+10*kHEIGHT/667.0, 150, 20)];
...
-(BOOL)identifyYourFaceLeft:(CGFloat)left right:(CGFloat)right top:(CGFloat)top bottom:(CGFloat)bottom
{
    ...
    if (isJudgeMouth != YES) {
        ...
        takePhotoNumber += 1;
        if (takePhotoNumber == 2 && !(timeCount>0&&timeCount<4)) {
...
-(void)identifyYourFaceOpenMouth:(NSString *)key p:(CGPoint )p
{
    ...
    if (rightX && leftX && upperY && lowerY && isJudgeMouth != YES) {
        ...
        if (mouthWidth && mouthWidthF) {
            //张嘴验证完毕
            if (mouthHeight - mouthHeightF >= 10 && mouthWidthF - mouthWidth >= 5) {
                isJudgeMouth = YES;
...
-(void)didClickTakePhoto
{
    ...
    [myStillImageOutput captureStillImageAsynchronouslyFromConnection:myVideoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        if (imageDataSampleBuffer) {
            ...
            //取得的静态影像
            myNewImage = [[UIImage alloc] initWithData:imageData];
...
-(void)didClickUpPhoto
{
    ...
    if (_faceDelegate) {
        ...
        _activityIndicator.center = CGPointMake(kWIDTH/2.0, kHEIGHT/2.0);
        [self.view addSubview:_activityIndicator];
        [_activityIndicator startAnimating];

        [[LCFaceRecognizeService faceRecognizeService] lc_faceRegistWithImage:_myImage complete:^(BOOL success ,id result) {
            NSDictionary *regDic = (NSDictionary *)result;
            NSString * gid = [regDic objectForKey:KCIFlyFaceResultGID];

            if (!success) {
                return;
            }
            [[LCFaceRecognizeService faceRecognizeService] lc_faceVerifyWithImage:myNewImage gid:gid complete:^(BOOL success ,id result) {
                if (success) {
                    NSDictionary *verDic = (NSDictionary *)result;
                    NSString* score=[verDic objectForKey:KCIFlyFaceResultScore];
                    if (_handle) {
                        _handle(score);
                    }
                }
...
-(void)delateNumber
{
    ...
    imgView.animationImages = nil;
    imgView.image = [UIImage imageNamed:@"shakeHead0.png"];
